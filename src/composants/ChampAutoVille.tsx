...
interface Props {
  codePostal: string;
  ville: string;
  onChange: (v: { codePostal: string; ville: string }) => void;
  disabled?: boolean;
}

export default function ChampAutoVille({ codePostal, ville, onChange, disabled }: Props) {
  const [suggestions, setSuggestions] = useState<Ville[]>([]);
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  function rechercherAvecDelai(type: 'cp' | 'nom', valeur: string) {
