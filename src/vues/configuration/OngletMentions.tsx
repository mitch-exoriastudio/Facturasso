...
import ChampSansAutofill from '@/composants/ChampSansAutofill';
...
// Clés de `ParametreGeneral` dont la valeur est textuelle : seules celles-là
// sont éditées par `maj` (affectation dynamique type-safe).
type ChampTexte = {
  [K in keyof ParametreGeneral]: ParametreGeneral[K] extends string | null ? K : never;
}[keyof ParametreGeneral];
...
interface Props {
  params: ParametreGeneral;
  onMaj: (p: ParametreGeneral) => void;
  onModifie?: (modifie: boolean) => void;
}

export default function OngletMentions({ params, onMaj, onModifie }: Props) {
  const [form, setForm] = useState<ParametreGeneral>({ ...params });
  const [reference, setReference] = useState<ParametreGeneral>({ ...params });
  ...
  const maj = (champ: ChampTexte, fmt?: 'MAJ' | '1ere') => (e: EvenementValeur) => {
  ...
  function chargerLogo(e: ChangeEvent<HTMLInputElement>) {
    const fichier = e.target.files?.[0];
    if (!fichier) return;
    const reader = new FileReader();
    reader.onload = () => setForm((f) => ({ ...f, logo_asso: reader.result as string }));
  ...
  async function sauvegarder(e: FormEvent) {
    ...
    try {
      ...
      setErreur(e2.response?.data?.message ?? 'Erreur lors de la sauvegarde.');
    }
