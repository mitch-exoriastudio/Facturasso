...
import type { SessionUtilisateur, UtilisateurConfig } from '@/types';

// Clés « droit » cochables du formulaire.
type CleDroitForm =
  | 'droit_admin' | 'droit_consult_fac' | 'droit_ajout_fac'
  | 'droit_consult_paiem' | 'droit_ajout_paiem'
  | 'droit_consult_clients' | 'droit_ajout_clients' | 'droit_config';

// État du formulaire de compte.
...
const DROITS: { cle: CleDroitForm; label: string }[] = [
...
const GROUPES: { titre: string; droits: { cle: CleDroitForm; label: string }[] }[] = [
...
const FORM_VIDE: FormUtilisateur = {
...
function estModifie(a: unknown, b: unknown): boolean {
...
function libelleRole(u: UtilisateurConfig): string {
...
interface Props {
  utilisateurConnecte: SessionUtilisateur | null;
  onModifie?: (modifie: boolean) => void;
}

export default function OngletUtilisateurs({ utilisateurConnecte, onModifie }: Props) {
  const toast = useToast();
  const [utilisateurs, setUtilisateurs] = useState<UtilisateurConfig[]>([]);
  const [avecDesactives, setAvecDesactives] = useState(false);
  const [selectionne, setSelectionne] = useState<UtilisateurConfig | null>(null); // utilisateur sélectionné
  const [form, setForm] = useState<FormUtilisateur>(FORM_VIDE);
  const [reference, setReference] = useState<FormUtilisateur | null>(null);      // état de référence (null = rien d'ouvert)
  const [message, setMessage] = useState('');
  const [estNouveau, setEstNouveau] = useState(false);
  const [montrerMdp, setMontrerMdp] = useState(false);
  const [enCours, setEnCours] = useState(false);
  const [actionEnAttente, setActionEnAttente] = useState<(() => void) | null>(null); // garde changement de sélection
  const [suppressionCible, setSuppressionCible] = useState<UtilisateurConfig | null>(null); // utilisateur en attente de suppression
  ...
  function peutSupprimer(u: UtilisateurConfig): boolean {
  ...
  async function charger(): Promise<UtilisateurConfig[]> {
  ...
  function ouvrirUtilisateur(u: UtilisateurConfig) {
    const f: FormUtilisateur = { ...u, email: u.email ?? '', telephone: u.telephone ?? '', mot_de_passe: '' };
    setSelectionne(u);
    setEstNouveau(false);
    setForm(f);
    setReference(f);
  ...
  function demanderSelection(u: UtilisateurConfig) {
  ...
  const majDroit = (cle: CleDroitForm) => (e: ChangeEvent<HTMLInputElement>) => {
    const f: FormUtilisateur = { ...form, [cle]: e.target.checked };
  ...
  async function sauvegarder(e: FormEvent) {
    ...
    try {
      if (estNouveau) {
        ...
        const data = await charger();
        const majUtilisateur = data.find((u) => u.id_utilisateur === selectionne.id_utilisateur);
        setSelectionne(majUtilisateur ?? selectionne);
        setReference({ ...form });
      }
      setMessage('Enregistré !');
      setTimeout(() => setMessage(''), 2000);
    } catch (err) {
      setMessage(messageErreur(err, 'Erreur lors de la sauvegarde.'));
    } finally {
      setEnCours(false);
    }
  }
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={form.mot_de_passe}
                      ...
                      style={{ WebkitTextSecurity: montrerMdp ? 'none' : 'disc' } as CSSProperties}
                      className={CL + ' pr-9'} />
