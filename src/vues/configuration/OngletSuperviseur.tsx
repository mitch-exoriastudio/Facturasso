...
interface Props {
  ...
  onMajNumero: (n: number) => void;
}

export default function OngletSuperviseur({ dernierNumero, onMajNumero }: Props) {
  const [numero, setNumero] = useState<number | string>(dernierNumero ?? 0);
  ...
  async function importerCsv(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setImportMsg('Erreur : ' + (e2.response?.data?.message || e2.message));
    }
  }
