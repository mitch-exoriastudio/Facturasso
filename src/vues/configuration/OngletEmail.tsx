...
import type { UtilisateurConfig } from '@/types';

const IconeOeil = ({ visible }: { visible: boolean }) => visible
...
// Événement minimal accepté par les champs (input comme textarea).
interface EvenementValeur {
  target: { value: string };
}

// État du formulaire e-mail (chaînes de saisie ; port éventuellement en texte).
interface FormEmail {
  smtp_adresse: string;
  smtp_user: string;
...
function normaliserForm(data: unknown): FormEmail {
  return { ...FORM_VIDE, smtp_mot_de_passe: '', ...(data as object || {}) };
}
...
export default function OngletEmail({ onModifie }: Props) {
  ...
  const [idEnAttente, setIdEnAttente] = useState<number | null>(null);
  const [form, setForm] = useState<FormEmail>(FORM_VIDE);
  const [reference, setReference] = useState<FormEmail>(FORM_VIDE);
  ...
  useEffect(() => {
    configService.getUtilisateurs({}).then((data: UtilisateurConfig[]) => {
  ...
  const maj = (champ: keyof FormEmail) => (e: EvenementValeur) =>
    setForm((f) => ({ ...f, [champ]: e.target.value }));

  function demanderChangementUtilisateur(id: number) {
    if (modifie) {
      setIdEnAttente(id);
    } else {
      setIdSelectionne(id);
  ...
  function chargerSignature(e: ChangeEvent<HTMLInputElement>) {
    const fichier = e.target.files?.[0];
    if (!fichier) return;
    const reader = new FileReader();
    reader.onload = () => setForm((f) => ({ ...f, signature_img: reader.result as string }));
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-6 p-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg">
        ...
        <MenuDeroulant
          value={idSelectionne ?? ''}
          onChange={(id) => demanderChangementUtilisateur(Number(id))}
          options={utilisateurs.map((u) => ({ value: u.id_utilisateur, label: u.nom_utilisateur }))}
...
function Champ({ label, value, onChange }: {
  ...
  onChange: (e: EvenementValeur) => void;
}) {
  return (
    <div>
