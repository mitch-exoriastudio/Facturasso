    ...
    <div className="max-w-4xl">
      ...
      {!chargement && idSelectionne && (
        <form onSubmit={sauvegarder} autoComplete="off">
          {message && <div className="text-sm text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20 rounded-lg p-3 mb-4">{message}</div>}
          {erreur && <div className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg p-3 mb-4">{erreur}</div>}
          ...
          <div className="grid grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Identifiant du compte e-mail</label>
                <input value={form.smtp_user} onChange={maj('smtp_user')} autoComplete="off" className={CL} />
              </div>
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">
                  Mot de passe <span className="text-gray-400 dark:text-gray-500 font-normal">(laisser vide = inchangé)</span>
                </label>
              ...
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Adresse du serveur SMTP</label>
                <input value={form.smtp_adresse} onChange={maj('smtp_adresse')} autoComplete="off" className={CL} />
              </div>
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Adresse e-mail de l'expéditeur</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">CC</label>
                  <input value={form.email_cc} onChange={maj('email_cc')} autoComplete="off" className={CL} />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">CCI</label>
              ...
              {/* Signature */}
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Image de la signature</label>
                <div className="flex items-start gap-3">
                  {form.signature_img
                    ? <img src={form.signature_img} alt="Signature" className="max-h-16 object-contain border rounded" />
                    : <div className="h-12 w-32 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded flex items-center justify-center text-xs text-gray-400 dark:text-gray-500">Aucune</div>
                  }
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-300">Modèle d'e-mail d'envoi de facture</h3>
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Objet</label>
          ...
          <div className="mt-6 flex items-center gap-3">
            ...
            <button type="button" disabled
              ...
              className="border border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-600 font-semibold px-6 py-2 rounded-lg cursor-not-allowed select-none">
